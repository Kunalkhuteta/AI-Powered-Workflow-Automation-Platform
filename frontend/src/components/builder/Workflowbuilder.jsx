...
const WorkflowBuilder = () => {
 ...
 const getDefaultConfig = (type) => {
  switch (type) {
    ...
    case 'conditional':
      return { left_value: '', operator: '==', right_value: '', true_output: { result: true }, false_output: { result: false } };

    case 'transform':
      return { operation: 'extract', fields: [], mapping: {}, source_node: '' };

    case 'delay':
      return { duration: 1, unit: 'seconds', reason: '' };

    case 'webhook':
      return { url: '', method: 'POST', body: {}, headers: {}, timeout: 30, include_inputs: false };

    case 'database':
      return {
        connection: {        // ← this was the crash culprit
          type: 'postgresql',
          host: '',
          port: 5432,
          database: '',
    ...
    case 'google_sheets':
      return { spreadsheet_id: '', sheet_name: '', operation: 'read', range: 'A1:Z1000' };

    case 'csv_excel':
      return { operation: 'read', file_path: '', delimiter: ',', has_header: true };
    ...
    (params) => {
      ...
      if (sourceNode && sourceNode.type === 'conditional') {
        ...
        if (handleId === 'true' || handleId.includes('true')) {
          ...
          edgeLabelStyle = { fill: '#10b981', fontWeight: 600, fontSize: '12px' };
        } else if (handleId === 'false' || handleId.includes('false')) {
          edgeLabel = 'false';
          edgeStyle = { stroke: '#ef4444', strokeWidth: 2 };
          edgeLabelStyle = { fill: '#ef4444', fontWeight: 600, fontSize: '12px' };
        } else {
